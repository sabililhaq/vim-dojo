...
import { challengeSets, vimChallenges, type Challenge } from '../src/challenges';
import {
  categoriesIn,
  createPlaylist,
  dailyChallenge,
...
describe('playlist query', () => {
  it('parses category, random, daily, and the ?daily alias', () => {
    expect(parseQuery('?category=motion&challenge=motion-02', categories)).toEqual({
      challenge: 'motion-02',
      category: 'motion',
      mode: 'sequence',
    });
    expect(parseQuery('?mode=random&category=operator', categories).mode).toBe('random');
  ...
  it('builds shareable URLs without a trailing empty query', () => {
    expect(playlistUrl('/vim', { mode: 'sequence', category: null, challenge: 'motion-01' })).toBe(
      '/vim?challenge=motion-01',
    );
    expect(
      playlistUrl('/vim/', { mode: 'random', category: 'motion', challenge: 'motion-02' }),
    ).toBe('/vim?mode=random&category=motion&challenge=motion-02');
    expect(playlistUrl('/vim', { mode: 'daily', category: 'motion', challenge: 'operator-01' })).toBe(
      '/vim?mode=daily&challenge=operator-01',
    );
    expect(playlistUrl('/', { mode: 'sequence', category: null, challenge: null })).toBe('/');
  });
});
...
describe('category play', () => {
  it('keeps Next/Previous inside the filtered set', () => {
    ...
    expect(playlist.items.map((challenge) => challenge.id)).toEqual(['motion-01', 'motion-02']);
    expect(playlist.index).toBe(0);
    expect(playlist.query.category).toBe('motion');
    expect(playlist.items.every((challenge) => challenge.category === 'motion')).toBe(true);
  });
  ...
  it('filters the real curriculum by category', () => {
    ...
    expect(playlist.items.every((challenge) => challenge.category === 'search')).toBe(true);
  });
});
...
describe('random review', () => {
  it('prefers unsolved cases and does not reshuffle a restored list', () => {
    ...
    expect(first.items.map((challenge) => challenge.id)).toEqual(['operator-01', 'motion-02']);
    ...
    expect(restored.items.map((challenge) => challenge.id)).toEqual(['operator-01', 'motion-02']);
  ...
  it('reshuffles only when asked, and shuffles the full set once everything is done', () => {
    ...
    expect(shuffled.items.map((challenge) => challenge.id)).toEqual(['motion-02', 'motion-01']);
  });
...
describe('daily kata', () => {
  it('picks one deterministic case per UTC day', () => {
    const morning = dailyChallenge(vimChallenges, new Date('2026-08-20T00:00:00.000Z'));
    const night = dailyChallenge(vimChallenges, new Date('2026-08-20T23:59:59.999Z'));

    expect(morning?.id).toBe(night?.id);
    expect(morning?.id).toBeTruthy();
    expect(hashString('2026-08-20')).not.toBe(hashString('2026-08-21'));
    expect(utcDateString(new Date('2026-08-20T12:00:00.000Z'))).toBe('2026-08-20');
...
describe('playlist helpers', () => {
  ...
  it('shuffles unsolved cases and falls back to the full set', () => {
    const unsolved = shuffleUnsolved(sample, ['motion-01', 'operator-01'], () => 0);
    expect(unsolved.map((challenge) => challenge.id)).toEqual(['motion-02']);

    const allDone = shuffleUnsolved(sample, ['motion-01', 'motion-02', 'operator-01'], () => 0);
