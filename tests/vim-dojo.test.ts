...
describe("Vim Dojo", () => {
  it("keeps every challenge complete, unique, and in its category set", () => {
    ...
    for (const [category, set] of Object.entries(challengeSets)) {
      ...
      expect(set.every((challenge) => challenge.category === category)).toBe(
        true,
      );
    }

    expect(vimChallenges).toHaveLength(
      Object.values(challengeSets).reduce(
  ...
  it("registers page controls as Vim Ex commands and aliases", () => {
    const source = readFileSync(mountPath, "utf-8");
    const template = readFileSync(templatePath, "utf-8");

    expect(source).toContain('Vim.defineEx("hint", "hi"');
    expect(source).toContain('Vim.defineEx("retry", "r"');
    expect(source).toContain('Vim.defineEx("previous", "p"');
    expect(source).toContain('Vim.defineEx("next", "n"');
    expect(source).toContain('Vim.defineEx("shuffle", "sh"');
    expect(source).toContain("function goToAnotherChallenge");
    expect(source).toContain("playlist.items[index]");
    expect(source).not.toContain('"shuffled"');
    expect(source).toContain("registerExCommands();");
    expect(template).toContain("Custom vim command for this page");
    expect(template).toContain("<code>:hint</code>");
    expect(template).toContain("<code>:retry</code>");
    expect(template).toContain("<code>:previous</code>");
    expect(template).toContain("<code>:next</code>");
    expect(template).toContain("<code>:shuffle</code>");
    expect(template).toContain("Jump to another challenge");
    expect(template).toContain("<code>:n</code>");
  });
  ...
  it("themes editor and buttons from host surface tokens instead of hardcoded white", () => {
    ...
    expect(styles).toContain(
      "--vd-surface: var(--surface, var(--bg, 255 255 255))",
    );
  ...
  it("themes the vim search panel from host tokens so dark mode input text stays readable", () => {
    ...
    expect(styles).toMatch(
      /\.cm-editor \.cm-panels \{[\s\S]*?background-color: rgb\(var\(--vd-surface\)\)/,
    );
    expect(styles).toMatch(
      /\.cm-vim-panel input[\s\S]*?color: rgb\(var\(--vd-gray-dark\)\)/,
    );
  });
  ...
  it("marks a passed case from vim-dojo:completed", () => {
    ...
    expect(template).toContain('class="completion-mark"');
    expect(template).toContain('title="Challenge not completed"');
    expect(source).toContain("function updatePassedMark");
    expect(source).toContain('passed ? "✓" : "○"');
    expect(source).toContain("Challenge completed");
  ...
  it("plays by category, random, and daily from the query string", () => {
    ...
    expect(template).not.toContain("data-modes");
    expect(template).toContain("data-shuffle-button");
    expect(template).toContain("Want a specific category?");
    expect(template.indexOf('class="actions"')).toBeLessThan(
      template.indexOf("data-toast"),
    );
    expect(template.indexOf("data-progress")).toBeLessThan(
      template.indexOf("data-categories"),
    );
    ...
    expect(source).toContain('els.shuffleButton?.removeAttribute("hidden")');
  ...
  it("classifies paste and manual attempts separately from Vim attempts", () => {
    ...
    expect(
      classifyAttempt(vimEvents, 'const x = "old";', 'const x = "new";'),
    ).toBe("vim");
    expect(
      classifyAttempt(deleteLineEvents, "a\nconsole.log(x);\nb", "a\nb"),
    ).toBe("vim");
    expect(
      classifyAttempt(manualEvents, 'const x = "old";', 'const x = "new";'),
    ).toBe("manual");
    expect(
      classifyAttempt(mouseOnlyEvents, "timeout: 3000", "timeout: 5000"),
    ).toBe("manual");
    expect(
      classifyAttempt(pasteEvents, 'const x = "old";', 'const x = "new";'),
    ).toBe("paste");
  });
  ...
  it("does not treat a tiny paste as the whole solution", () => {
    ...
    expect(
      classifyAttempt(events, 'const x = "oldvalue";', 'const x = "newvalue";'),
    ).toBe("vim");
  });
  ...
  it("classifies a single click as mostly-vim and extra mouse work as mixed", () => {
    ...
    expect(
      classifyAttempt(mostlyVimEvents, 'const x = "old";', 'const x = "new";'),
    ).toBe("mostly-vim");
  ...
  it("uses a visual change sequence for the visual challenge", () => {
    const visual = vimChallenges.find(
      (challenge) => challenge.id === "visual-01",
    );
  ...
  it("keeps intended moves for the new practice cases", () => {
    const byId = Object.fromEntries(
      vimChallenges.map((challenge) => [challenge.id, challenge]),
    );
    ...
    expect(byId["visual-08"]?.intendedMove).toBe("<C-v>jjr");
...
describe("Vim Dojo learning", () => {
  ...
  it("teaches with progressive hints that finish on the intended keys", () => {
    for (const challenge of vimChallenges) {
      ...
      expect(lastHintKeys(lastHint), challenge.id).toContain(
        challenge.intendedMove ?? "",
      );
    }
  });
  ...
  it("covers a core motion, operator, text-object, and visual curriculum", () => {
    ...
    const concepts = vimChallenges.flatMap(
      (challenge) => challenge.concepts ?? [],
    );
    ...
    expect(
      challengeSets.motion.some((challenge) =>
        challenge.intendedMove?.includes("b"),
      ),
    ).toBe(true);
    expect(
      challengeSets.motion.some((challenge) =>
        challenge.intendedMove?.includes("t"),
      ),
    ).toBe(true);
    expect(
      challengeSets.motion.some((challenge) =>
        challenge.intendedMove?.includes("F"),
      ),
    ).toBe(true);
    expect(
      challengeSets.motion.some((challenge) =>
        challenge.intendedMove?.includes("e"),
      ),
    ).toBe(true);
    expect(
      challengeSets.motion.some((challenge) =>
    ...
    expect(
      challengeSets.motion.some((challenge) =>
        challenge.intendedMove?.includes("%"),
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "x",
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "D",
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "C",
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === 'dt"',
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "r",
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "cc",
      ),
    ).toBe(true);
    expect(
      challengeSets["text-object"].some(
        (challenge) => challenge.intendedMove === "daw",
      ),
    ).toBe(true);
    expect(
      challengeSets["text-object"].some(
        (challenge) => challenge.intendedMove === 'ca"',
      ),
    ).toBe(true);
    expect(
      challengeSets["text-object"].some(
        (challenge) => challenge.intendedMove === "ci'",
      ),
    ).toBe(true);
    expect(
      challengeSets.visual.some(
        (challenge) => challenge.intendedMove === 'vi"c',
      ),
    ).toBe(true);
    expect(
      challengeSets.visual.some(
        (challenge) => challenge.intendedMove === "viwc",
      ),
    ).toBe(true);
    expect(
      challengeSets.visual.some(
    ...
    expect(
      challengeSets.motion.some(
        (challenge) => challenge.intendedMove === "Gcw",
      ),
    ).toBe(true);
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "Gdd",
      ),
    ).toBe(true);
    expect(
      challengeSets["text-object"].some(
        (challenge) => challenge.intendedMove === 'Gci"',
      ),
    ).toBe(true);
    expect(
      challengeSets.visual.some(
        (challenge) => challenge.intendedMove === "GVkkd",
      ),
    ).toBe(true);
    expect(
      challengeSets.search.some(
        (challenge) => challenge.intendedMove === "/legacycw",
      ),
    ).toBe(true);
    expect(
      challengeSets.search.some(
        (challenge) => challenge.intendedMove === '/betanci"',
      ),
    ).toBe(true);
    expect(
      challengeSets.search.some(
        (challenge) => challenge.intendedMove === "*cw",
      ),
    ).toBe(true);
    expect(
      challengeSets.search.some(
    ...
    expect(
      challengeSets.replace.some(
        (challenge) => challenge.intendedMove === ":s/colour/color",
      ),
    ).toBe(true);
    expect(
      challengeSets.replace.some(
        (challenge) => challenge.intendedMove === ":%s/colour/color/g",
      ),
    ).toBe(true);
    expect(
      challengeSets.replace.some(
    ...
    expect(
      challengeSets.operator.some(
        (challenge) => challenge.intendedMove === "xp",
      ),
    ).toBe(true);
    expect(
      challengeSets["text-object"].some(
    ...
    expect(
      challengeSets.visual.some(
        (challenge) => challenge.intendedMove === "<C-v>jjr",
      ),
    ).toBe(true);
    expect(
      challengeSets.visual.some(
        (challenge) => challenge.intendedMove === "Vjd",
      ),
    ).toBe(true);
    expect(
      challengeSets.register.some(
        (challenge) => challenge.intendedMove === '"ayyjdd"ap',
      ),
    ).toBe(true);
    expect(
      challengeSets.register.some(
        (challenge) => challenge.intendedMove === '"ayiwjwciw<C-r>a',
      ),
    ).toBe(true);
    expect(
      challengeSets.mark.some(
        (challenge) => challenge.intendedMove === "maggdd'acw",
      ),
    ).toBe(true);
    expect(
      challengeSets.macro.some(
        (challenge) => challenge.intendedMove === "qadwjq2@a",
      ),
    ).toBe(true);
    expect(
      challengeSets.format.some((challenge) => challenge.intendedMove === ">>"),
    ).toBe(true);
    expect(
      challengeSets.format.some((challenge) => challenge.intendedMove === "=="),
    ).toBe(true);
    expect(
      challengeSets.format.some((challenge) => challenge.intendedMove === ">j"),
    ).toBe(true);
    expect(
      challengeSets["multi-cursor"].some(
        (challenge) => challenge.intendedMove === "<C-v>jjI",
      ),
    ).toBe(true);
    expect(
      challengeSets["multi-cursor"].some(
        (challenge) => challenge.intendedMove === "$<C-v>jjA",
      ),
    ).toBe(true);
  });
  ...
  it("places the cursor so the intended move is the natural first action", () => {
    for (const challenge of vimChallenges) {
      ...
      expect(
        challenge.initialCursor!.line,
        challenge.id,
      ).toBeGreaterThanOrEqual(0);
      expect(
        challenge.initialCursor!.column,
        challenge.id,
      ).toBeGreaterThanOrEqual(0);
    }
  });
  ...
  it("keeps most buffers short, and viewport cases long enough to scroll", () => {
    ...
    for (const challenge of vimChallenges) {
      ...
      if (viewportIds.has(challenge.id)) {
        ...
        expect(challenge.intendedMove, challenge.id).toMatch(
          /^(G|gg|\/|\*|:%s)/,
        );
        continue;
      }
  ...
  it("points beginners to VimHero and exposes a hint button", () => {
    ...
    expect(source).toContain(
      "You solved it, but you missed some Vim practice.",
    );
    expect(source).toContain(
      "One mouse interaction nudged this into mostly Vim.",
    );
  });

  it("teaches daw from the middle of unused, where dw would leave a prefix", () => {
    const challenge = vimChallenges.find(
      (entry) => entry.id === "text-object-05",
    );
    const unusedAt = challenge?.initialContent.indexOf("unused") ?? -1;

    expect(challenge?.initialCursor?.column).toBeGreaterThan(unusedAt);
    expect(challenge?.initialCursor?.column).toBeLessThan(
      unusedAt + "unused".length,
    );
  ...
  it("publishes a learning roadmap for category, random, daily, and interactive hints", () => {
    ...
    const roadmapPath = fileURLToPath(
  ...
  it("finds true from after the closing paren, where f would miss it", () => {
    ...
    expect(challenge?.initialCursor?.column).toBeGreaterThan(
      trueAt + "true".length - 1,
    );
  ...
  it("changes the rest of the line with C, not a single word", () => {
    ...
    expect(challenge?.initialCursor?.column).toBe(
      challenge?.initialContent.indexOf("user"),
    );
  ...
  it("deletes till the quote and leaves the filename quoted", () => {
    ...
    expect(challenge?.initialCursor?.column).toBe(
      challenge?.initialContent.indexOf("path"),
    );
  });
  ...
  it("starts on the opening paren so % jumps to the match before append", () => {
    ...
    expect(challenge?.initialCursor?.column).toBe(
      challenge?.initialContent.indexOf("("),
    );
  ...
  it("replaces the wrong digit in place with r", () => {
    ...
    expect(challenge?.initialCursor?.column).toBe(
      challenge?.initialContent.indexOf("1"),
    );
  ...
  it("teaches single-quote text objects as the pair of double quotes", () => {
    const doubles = vimChallenges.find(
      (entry) => entry.id === "text-object-01",
    );
    const singles = vimChallenges.find(
      (entry) => entry.id === "text-object-07",
    );
  ...
  it("changes inside brackets as the pair of parentheses and braces", () => {
    ...
    const brackets = vimChallenges.find(
      (entry) => entry.id === "text-object-09",
    );
  ...
  it("substitutes every match on the line, not just the first", () => {
    ...
    expect(lineGlobal?.initialContent.match(/colour/g)?.length).toBeGreaterThan(
      1,
    );
  ...
  it("replaces a visual-block column of flags", () => {
    const challenge = vimChallenges.find((entry) => entry.id === "visual-08");

    expect(challenge?.initialContent.split("\n")).toHaveLength(3);
    expect(challenge?.initialCursor?.column).toBe(
      challenge?.initialContent.indexOf("0"),
    );
    expect(challenge?.targetContent).toBe(
      ["timeout: 1,", "retries: 1,", "workers: 1,"].join("\n"),
    );
    expect(challenge?.concepts).toEqual(["Ctrl-v", "r"]);
  });

  it("parks a yank in a named register so a delete does not clobber it", () => {
    const challenge = vimChallenges.find((entry) => entry.id === "register-01");
    ...
    expect(
      challenge?.targetContent
        .split("\n")
        .filter((line) => line.includes("retries")),
    ).toHaveLength(2);
    expect(challenge?.concepts).toEqual(['"', "yy", "p"]);
  });

  it("returns to a mark after deleting an earlier line", () => {
    const challenge = vimChallenges.find((entry) => entry.id === "mark-01");

    expect(challenge?.initialCursor?.line).toBe(1);
    expect(challenge?.initialContent.split("\n")[1]).toContain("start");
    expect(challenge?.targetContent).toBe(["boot();", "finish();"].join("\n"));
    expect(challenge?.concepts).toContain("m");
  });

  it("records a line-local edit and replays it as a macro", () => {
    const challenge = vimChallenges.find((entry) => entry.id === "macro-01");

    expect(
      challenge?.initialContent
        .split("\n")
        .every((line) => line.startsWith("flag ")),
    ).toBe(true);
    expect(
      challenge?.targetContent
        .split("\n")
        .some((line) => line.startsWith("flag ")),
    ).toBe(false);
    expect(challenge?.concepts).toEqual(["q", "@", "dw"]);
  });

  it("indents with >> and reindents with ==", () => {
    const shift = vimChallenges.find((entry) => entry.id === "format-01");
    const auto = vimChallenges.find((entry) => entry.id === "format-02");
    const two = vimChallenges.find((entry) => entry.id === "format-03");

    expect(shift?.initialContent).toContain("\nreturn boot();");
    expect(shift?.targetContent).toContain("\n  return boot();");
    expect(auto?.initialContent.startsWith("  ")).toBe(true);
    expect(auto?.targetContent.startsWith("const")).toBe(true);
    expect(two?.intendedMove).toBe(">j");
  });

  it("inserts and appends on every line of a visual block", () => {
    const prefix = vimChallenges.find(
      (entry) => entry.id === "multi-cursor-01",
    );
    const suffix = vimChallenges.find(
      (entry) => entry.id === "multi-cursor-02",
    );

    expect(
      prefix?.targetContent
        .split("\n")
        .every((line) => line.startsWith("req.")),
    ).toBe(true);
    expect(
      suffix?.targetContent.split("\n").every((line) => line.endsWith(",")),
    ).toBe(true);
  ...
  it("returns to the previous position after an end-of-file edit", () => {
    ...
    expect(challenge?.targetContent).toBe(
      ["delay = 3000;", "retries = 3;", "kept = 1;"].join("\n"),
    );
    expect(challenge?.concepts).toContain("''");
  });
  ...
  it("records an insert-at-start macro and replays it down the file", () => {
    ...
    expect(
      challenge?.targetContent
        .split("\n")
        .every((line) => line.startsWith("const ")),
    ).toBe(true);
  ...
  it("outdents with << and indents through the end with >G", () => {
    ...
    expect(rest?.targetContent).toBe(
      ["return {", "  ok: true,", "  };"].join("\n"),
    );
  });
  ...
  it("changes a visual-block column to a longer value", () => {
    const challenge = vimChallenges.find(
      (entry) => entry.id === "multi-cursor-03",
    );

    expect(
      challenge?.targetContent
        .split("\n")
        .every((line) => line.endsWith("null")),
    ).toBe(true);
  ...
  it("jumps to the top of a long buffer with gg", () => {
    ...
    expect(challenge?.initialCursor?.line).toBe(
      challenge!.initialContent.split("\n").length - 1,
    );
